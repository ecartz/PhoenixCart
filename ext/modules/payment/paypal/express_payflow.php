...
EOD;
            ...
            $hooks->call('siteWide', 'postRegistration');
          }
...
SELECT address_book_id
 ...
 WHERE customers_id = %d AND entry_firstname = '%s' AND entry_lastname = '%s'
   AND entry_street_address = '%s' AND entry_postcode = '%s' AND entry_city = '%s'
   AND (entry_state = '" . . "' OR entry_zone_id = " .  . ")
   AND entry_country_id = %d
 LIMIT 1
EOSQL
          , (int)$_SESSION['customer_id'], $db->escape($ship_firstname), $db->escape($ship_lastname),
          $db->escape($ship_address), $db->escape($ship_postcode), $db->escape($ship_city),
          $db->escape($ship_zone), (int)$ship_zone_id, (int)$ship_country_id));
        if ($check = $check_query->fetch_assoc()) {
...
/* useraction=commit       Href::redirect($Linker->build('checkout_process.php')); */
        ...
        Href::redirect($Linker->build('shopping_cart.php', ['error_message' => $response_array['PHOENIX_ERROR_MESSAGE']]));
      }
...
//      }
      ...
      foreach ($order->products as $product) {
        $product_price = $GLOBALS['currencies']->format_raw(Tax::price($product['final_price'], $product['tax']));
      ...
      $params['BILLTOSTATE'] = Zone::fetch_code($order->billing['country']['id'], $order->billing['zone_id'], $order->billing['state']);
      ...
      if (!Text::is_empty($order->delivery['street_address'])) {
        ...
        $params['SHIPTOSTATE'] = Zone::fetch_code($order->delivery['country']['id'], $order->delivery['zone_id'], $order->delivery['state']);
...
// load all enabled shipping modules
      ...
      foreach ($quotes as $quote) {
        $shipping_rate = $GLOBALS['currencies']->format_raw(Tax::add($quote['cost'], $quote['tax']));
...
// set shipping for order total calculations; shipping in $item_params includes taxes
      if (isset($default_shipping)) {
        ...
        $order->info['shipping_cost'] = $GLOBALS['currencies']->format_raw(
                                          Tax::add($quotes[$default_shipping]['cost'], $quotes[$default_shipping]['tax']));
...
// Remove shipping tax from total that was added again in ot_shipping
      ...
      $params['AMT'] = $GLOBALS['currencies']->format_raw($order->info['total']);

      // safely pad higher for dynamic shipping rates (eg, USPS express)
      $item_params['MAXAMT'] = $GLOBALS['currencies']->format_raw($params['AMT'] + $expensive_rate + 100, true, '', 1);
