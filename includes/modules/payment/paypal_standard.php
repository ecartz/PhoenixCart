...
*/
  ...
  class paypal_standard {
    ...
    public function __construct() {
      ...
      $link = isset($GLOBALS['Admin']) ? $GLOBALS['Admin']->link('paypal.php', 'action=configure&module=PS') : '';
      $this->description = '<div align="center">' . $this->_app->drawButton($this->_app->getDef('module_ps_legacy_admin_app_button'), $link, 'primary') . '</div>';
      $this->sort_order = defined('PAYPAL_APP_PS_SORT_ORDER') ? PAYPAL_APP_PS_SORT_ORDER : 0;
      $this->enabled = defined('PAYPAL_APP_PS_STATUS') && in_array(PAYPAL_APP_PS_STATUS, ['1', '0']);
      $this->order_status = defined('PAYPAL_APP_PS_PREPARE_ORDER_STATUS_ID') && ((int)PAYPAL_APP_PS_PREPARE_ORDER_STATUS_ID > 0) ? (int)PAYPAL_APP_PS_PREPARE_ORDER_STATUS_ID : 0;

      if ( defined('PAYPAL_APP_PS_STATUS') ) {
        if ( PAYPAL_APP_PS_STATUS == '0' ) {
          $this->title .= ' [Sandbox]';
          $this->public_title .= ' (' . $this->code . '; Sandbox)';
        }

        if ( PAYPAL_APP_PS_STATUS == '1' ) {
      ...
      if ( $this->enabled === true ) {
        if ( !defined('PAYPAL_APP_PS_PDT_IDENTITY_TOKEN') || (Text::is_empty(PAYPAL_APP_PS_PDT_IDENTITY_TOKEN) && !$this->_app->hasCredentials('PS')) ) {
...
// has already been deducted in the IPN to avoid a quantity == 0 redirect
    ...
    public function update_status() {
      ...
      if ( ($this->enabled == true) && ((int)PAYPAL_APP_PS_ZONE > 0) ) {
        $check_query = $GLOBALS['db']->query("SELECT zone_id FROM zones_to_geo_zones WHERE geo_zone_id = " . (int)PAYPAL_APP_PS_ZONE . " AND zone_country_id = " . (int)$order->billing['country']['id'] . " ORDER BY zone_id");
...
// remove shipping tax in total tax value
      ...
      $parameters = [
        ...
        'shipping_1' => $GLOBALS['currencies']->format_raw($order->info['shipping_cost']),
        'business' => $this->_app->getCredentials('PS', 'email'),
        'amount_1' => $GLOBALS['currencies']->format_raw($order->info['total'] - $order->info['shipping_cost'] - $total_tax),
        'currency_code' => $_SESSION['currency'],
        'invoice' => $this->extract_order_id(),
        'custom' => $_SESSION['customer_id'],
        'notify_url' => $GLOBALS['Linker']->build('ext/modules/payment/paypal/standard_ipn.php', (isset($ipn_language) ? 'language=' . $ipn_language : ''), 'SSL', false, false),
        'rm' => '2',
        ...
        'paymentaction' => (PAYPAL_APP_PS_TRANSACTION_METHOD == '1') ? 'sale' : 'authorization',
      ];
...
// order totals are processed on checkout confirmation but not captured into a variable
      ...
      if ( ($has_negative_price == false) && ($GLOBALS['currencies']->format_raw($paypal_item_total) == $GLOBALS['currencies']->format_raw($order->info['total'])) ) {
        $parameters = array_merge($parameters, $item_params);
      } else {
        $parameters['tax_cart'] = $GLOBALS['currencies']->format_raw($total_tax);
      }

      if ( PAYPAL_APP_PS_EWP_STATUS == '1' ) {
        $parameters['cert_id'] = PAYPAL_APP_PS_EWP_PUBLIC_CERT_ID;
        ...
        $fp = fopen(PAYPAL_APP_PS_EWP_WORKING_DIRECTORY . '/' . $random_string . 'data.txt', 'w');
        fwrite($fp, $data);
        fclose($fp);
        ...
        if (function_exists('openssl_pkcs7_sign') && function_exists('openssl_pkcs7_encrypt')) {
          openssl_pkcs7_sign(PAYPAL_APP_PS_EWP_WORKING_DIRECTORY . '/' . $random_string . 'data.txt', PAYPAL_APP_PS_EWP_WORKING_DIRECTORY . '/' . $random_string . 'signed.txt', file_get_contents(PAYPAL_APP_PS_EWP_PUBLIC_CERT), file_get_contents(PAYPAL_APP_PS_EWP_PRIVATE_KEY), ['From' => $this->_app->getCredentials('PS', 'email')], PKCS7_BINARY);

          unlink(PAYPAL_APP_PS_EWP_WORKING_DIRECTORY . '/' . $random_string . 'data.txt');

// remove headers from the signature
          $signed = file_get_contents(PAYPAL_APP_PS_EWP_WORKING_DIRECTORY . '/' . $random_string . 'signed.txt');
          $signed = explode("\n\n", $signed);
          $signed = base64_decode($signed[1]);

          $fp = fopen(PAYPAL_APP_PS_EWP_WORKING_DIRECTORY . '/' . $random_string . 'signed.txt', 'w');
          fwrite($fp, $signed);
          fclose($fp);

          unset($signed);

          openssl_pkcs7_encrypt(PAYPAL_APP_PS_EWP_WORKING_DIRECTORY . '/' . $random_string . 'signed.txt', PAYPAL_APP_PS_EWP_WORKING_DIRECTORY . '/' . $random_string . 'encrypted.txt', file_get_contents(PAYPAL_APP_PS_EWP_PAYPAL_CERT), ['From' => $this->_app->getCredentials('PS', 'email')], PKCS7_BINARY);

          unlink(PAYPAL_APP_PS_EWP_WORKING_DIRECTORY . '/' . $random_string . 'signed.txt');

// remove headers from the encrypted result
          $data = file_get_contents(PAYPAL_APP_PS_EWP_WORKING_DIRECTORY . '/' . $random_string . 'encrypted.txt');
          $data = explode("\n\n", $data);
          $data = '-----BEGIN PKCS7-----' . "\n" . $data[1] . "\n" . '-----END PKCS7-----';

          unlink(PAYPAL_APP_PS_EWP_WORKING_DIRECTORY . '/' . $random_string . 'encrypted.txt');
        } else {
          exec(PAYPAL_APP_PS_EWP_OPENSSL . ' smime -sign -in ' . PAYPAL_APP_PS_EWP_WORKING_DIRECTORY . '/' . $random_string . 'data.txt -signer ' . PAYPAL_APP_PS_EWP_PUBLIC_CERT . ' -inkey ' . PAYPAL_APP_PS_EWP_PRIVATE_KEY . ' -outform der -nodetach -binary > ' . PAYPAL_APP_PS_EWP_WORKING_DIRECTORY . '/' . $random_string . 'signed.txt');
          unlink(PAYPAL_APP_PS_EWP_WORKING_DIRECTORY . '/' . $random_string . 'data.txt');

          exec(PAYPAL_APP_PS_EWP_OPENSSL . ' smime -encrypt -des3 -binary -outform pem ' . PAYPAL_APP_PS_EWP_PAYPAL_CERT . ' < ' . PAYPAL_APP_PS_EWP_WORKING_DIRECTORY . '/' . $random_string . 'signed.txt > ' . PAYPAL_APP_PS_EWP_WORKING_DIRECTORY . '/' . $random_string . 'encrypted.txt');
          unlink(PAYPAL_APP_PS_EWP_WORKING_DIRECTORY . '/' . $random_string . 'signed.txt');

          $fh = fopen(PAYPAL_APP_PS_EWP_WORKING_DIRECTORY . '/' . $random_string . 'encrypted.txt', 'rb');
          $data = fread($fh, filesize(PAYPAL_APP_PS_EWP_WORKING_DIRECTORY . '/' . $random_string . 'encrypted.txt'));
          fclose($fh);
    ...
    public function pre_before_check() {
      ...
      if ( (isset($_POST['receiver_email']) && in_array($_POST['receiver_email'], $seller_accounts)) || (isset($_POST['business']) && in_array($_POST['business'], $seller_accounts)) ) {
        ...
        $this->_app->log('PS', '_notify-validate', ($result == 'VERIFIED') ? 1 : -1, $pptx_params, $result, (PAYPAL_APP_PS_STATUS == '1') ? 'live' : 'sandbox');
      } elseif ( isset($_GET['tx']) ) { // PDT
        if ( !Text::is_empty(PAYPAL_APP_PS_PDT_IDENTITY_TOKEN) ) {
          $pptx_params['cmd'] = '_notify-synch';

          $parameters = 'cmd=_notify-synch&tx=' . urlencode(stripslashes($_GET['tx'])) . '&at=' . urlencode(PAYPAL_APP_PS_PDT_IDENTITY_TOKEN);
          ...
          $this->_app->log('PS', $pptx_params['cmd'], ($result == 'VERIFIED') ? 1 : -1, $pptx_params, $result, (PAYPAL_APP_PS_STATUS == '1') ? 'live' : 'sandbox');
        } else {
          $details = $this->_app->getApiResult('APP', 'GetTransactionDetails', ['TRANSACTIONID' => stripslashes($_GET['tx'])], (PAYPAL_APP_PS_STATUS == '1') ? 'live' : 'sandbox');
        ...
        $this->_app->log('PS', 'UNKNOWN', ($result == 'VERIFIED') ? 1 : -1, $pptx_params, $result, (PAYPAL_APP_PS_STATUS == '1') ? 'live' : 'sandbox');
      }
...
// load the after_process function from the payment modules
    ...
    public function check() {
      $check_query = $GLOBALS['db']->query("SELECT configuration_value FROM configuration WHERE configuration_key = 'PAYPAL_APP_PS_STATUS'");
    ...
    public function install() {
      Href::redirect(Guarantor::ensure_global('Admin')->link('paypal.php', 'action=configure&subaction=install&module=PS'));
    }

    public function remove() {
      Href::redirect(Guarantor::ensure_global('Admin')->link('paypal.php', 'action=configure&subaction=uninstall&module=PS'));
    }

    public function keys() {
      return ['PAYPAL_APP_PS_SORT_ORDER'];
    }
    ...
    public function verifyTransaction($pptx_params, $is_ipn = false) {
      if ( isset($pptx_params['invoice']) && is_numeric($pptx_params['invoice']) && ($pptx_params['invoice'] > 0) && isset($pptx_params['custom']) && is_numeric($pptx_params['custom']) && ($pptx_params['custom'] > 0) ) {
        ...
        if ( mysqli_num_rows($order_query) === 1 ) {
          ...
          if ( $pptx_params['mc_gross'] != $GLOBALS['currencies']->format_raw($total['value'], $order['currency'], $order['currency_value']) ) {
            $comment_status .= "\n" . 'Error Total Mismatch: PayPal transaction value (' . htmlspecialchars($pptx_params['mc_gross']) . ') does not match order value (' . $GLOBALS['currencies']->format_raw($total['value'], $order['currency'], $order['currency_value']) . ')';
          }
